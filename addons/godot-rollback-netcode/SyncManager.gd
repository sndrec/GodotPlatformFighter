...
func _ready() -> void:
	...
	_ping_timer.connect("timeout", Callable(self, "_on_ping_timer_timeout"))
	add_child(_ping_timer)
	
	_spawn_manager = SpawnManager.new()
	_spawn_manager.name = "SpawnManager"
	add_child(_spawn_manager)
	_spawn_manager.connect("scene_spawned", Callable(self, "_on_SpawnManager_scene_spawned"))
	_spawn_manager.connect("scene_despawned", Callable(self, "_on_SpawnManager_scene_despawned"))
...
func set_network_adaptor(_network_adaptor: Object) -> void:
	...
	if network_adaptor != null:
		...
		network_adaptor.disconnect("received_ping", Callable(self, "_on_received_ping"))
		network_adaptor.disconnect("received_ping_back", Callable(self, "_on_received_ping_back"))
		network_adaptor.disconnect("received_remote_start", Callable(self, "_on_received_remote_start"))
		network_adaptor.disconnect("received_remote_stop", Callable(self, "_on_received_remote_stop"))
		network_adaptor.disconnect("received_input_tick", Callable(self, "_on_received_input_tick"))
	...
	network_adaptor.connect("received_ping", Callable(self, "_on_received_ping"))
	network_adaptor.connect("received_ping_back", Callable(self, "_on_received_ping_back"))
	network_adaptor.connect("received_remote_start", Callable(self, "_on_received_remote_start"))
	network_adaptor.connect("received_remote_stop", Callable(self, "_on_received_remote_stop"))
	network_adaptor.connect("received_input_tick", Callable(self, "_on_received_input_tick"))
...
func add_peer(peer_id: int) -> void:
	...
	assert(peer_id != network_adaptor.get_unique_id(), "Cannot add ourselves as a peer in SyncManager")
	
	if peers.has(peer_id):
		return
	if peer_id == network_adaptor.get_unique_id():
		return
...
func _on_ping_timer_timeout() -> void:
	if peers.size() == 0:
		return
	var msg = {
		local_time = int(Time.get_unix_time_from_system() * 1000)
	}
	for peer_id in peers:
		assert(peer_id != network_adaptor.get_unique_id(), "Cannot ping ourselves")
		network_adaptor.send_ping(peer_id, msg)

func _on_received_ping(peer_id: int, msg: Dictionary) -> void:
	assert(peer_id != network_adaptor.get_unique_id(), "Cannot ping back ourselves")
	msg['remote_time'] = int(Time.get_unix_time_from_system() * 1000)
...
func start_logging(log_file_path: String, match_info: Dictionary = {}) -> void:
	...
	#if _logger.start(Callable(log_file_path, network_adaptor.get_unique_id()).bind(match_info)) != OK:
	#	stop_logging()
...
func _send_input_messages_to_peer(peer_id: int) -> void:
	assert(peer_id != network_adaptor.get_unique_id(), "Cannot send input to ourselves")
	var peer = peers[peer_id]
...
func _physics_process(_delta: float) -> void:
	...
	if not mechanized:
		...
		input_frame.players[network_adaptor.get_unique_id()] = InputForPlayer.new(local_input, false)
