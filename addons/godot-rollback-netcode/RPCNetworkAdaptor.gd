...
@rpc("any_peer") func _rit(msg: PackedByteArray) -> void:
	emit_signal("received_input_tick", get_tree().get_multiplayer().get_remote_sender_id(), msg)
